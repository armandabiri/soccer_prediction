...
Player goal and assist allocation uses per-appearance production rather than raw career totals, with position-specific empirical priors to stabilize small samples. When a player source supplies `recent_appearances`, `recent_goals`, and `recent_assists` for up to 20 games, recent form receives a bounded 55% blend; otherwise reports clearly mark an up-to-20 equivalent estimated from aggregate totals. A source can instead supply `recent_games` — a per-match `[played, goals, assists]` timeline — from which those recent aggregates are derived, and which also drives a **recent-availability factor**: production is scaled by how many of the last five matches the player actually featured in, floored at 0.15 (they can still appear off the bench). Whether a player started the recent games is one of the strongest signals for whether they play, and score, in the next one, so an absentee is damped and cedes goal/assist share to teammates who are playing. The HTML report shows, next to each player, a strip of ten circles (one per recent game, oldest first) — the number is goals+assists that game and a solid grey circle marks a game missed — alongside the per-player form bars and an all-player comparison chart. The model exposes separate score, assist, score-or-assist, and first-scorer probabilities. Expected assists are limited to a 72% assisted-goal share so assist markets do not assume every goal has a credited assist. These estimates are availability-weighted but still not a full lineup/minutes model.
